...
namespace Internationalization
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string dob = ReadLine();
            Write("Enter your desired salary: ");
            string desiredSalary = ReadLine();
            DateTime date = DateTime.Parse(dob);
            int minutes = (int)DateTime.Today.Subtract(date).TotalMinutes;
            decimal toEarn = decimal.Parse(desiredSalary);
            WriteLine("{0} was born on a {1:dddd}, is {2:N0} minutes old and would like to earn {3:C}",
                name, date, minutes, toEarn
            );
        }
    }
}
